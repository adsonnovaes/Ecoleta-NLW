...
// request body : criação e edição, pra preencher os dados

// query params: para filtros e paginações
class PointsController{
    ...
    async show(request: Request,response: Response) {
        ...
        if(!point){
            return response.status(400).json({mensege: 'Point not found.'});
        }
    ...
    async create (request: Request,response: Response) {
        const {
            name,email, whatsapp, latitude, longitude,
            city, uf, items
        } = request.body;
        ...
        const point = {
            image: request.file.filename, name, email, whatsapp,
